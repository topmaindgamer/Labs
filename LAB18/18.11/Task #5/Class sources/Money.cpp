...
Money& Money::operator-=(Money& subtractor)
{
    ...
    if (_cents < subtractor.Cents())
    {
        dollarsOffset = -1;
        centsOffset = 100;
    }

    this->SetCents(_cents - subtractor.Cents() + centsOffset);
    this->SetDollars(_dollars - subtractor.Dollars() + dollarsOffset);
...
{
    this->SetDollars(_dollars / divider);
    this->SetCents(_cents / divider);
